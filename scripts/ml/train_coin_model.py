...
class SymbolSpecificTrainer:
    def __init__(self, symbol, lookback_days=60, prediction_days=1):
        ...
        self.symbol_configs = {
            'COIN': {'epochs': 150, 'batch_size': 16, 'lstm_units': [120, 60, 30], 'dropout': 0.35},  # High volatility crypto stock
    ...
    def get_symbol_specific_features(self, data):
        ...
        # Volatility features (important for crypto-related stocks)
        ...
        # Additional features for high-volatility stocks
    ...
    def train(self, data_period='2y'):
        ...
        early_stopping = EarlyStopping(
            monitor='val_loss',
            patience=25,  # More patience for volatile stocks
        ...
        reduce_lr = ReduceLROnPlateau(
            monitor='val_loss',
            factor=0.5,
            patience=15,  # More patience for volatile stocks
            min_lr=0.0001
        )
    ...
    def save_model(self, model_dir='persistent_data/ml_models', scaler_dir='persistent_data/scalers'):
        ...
        # Save scaler
        scaler_path = os.path.join(scaler_dir, f'{self.symbol.lower()}_scaler.pkl')
        joblib.dump(self.scaler, scaler_path)
        print(f"Scaler saved to: {scaler_path}")
        
        # Save metadata
        metadata_path = os.path.join(model_dir, f'{self.symbol.lower()}_metadata.json')
