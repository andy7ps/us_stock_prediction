#!/usr/bin/env python3
"""
Fixed Symbol-Specific LSTM Model Training
...
class SymbolSpecificTrainer:
    ...
    def get_symbol_specific_features(self, data):
        """Add symbol-specific technical indicators with pandas compatibility"""
        df = data.copy()
        ...
        df['RSI'] = rsi_calc.fillna(50)  # Default to neutral RSI
        ...
        # Bollinger Bands (fixed for pandas compatibility)
        df['BB_Middle'] = df['Close'].rolling(window=20).mean()
        bb_std = df['Close'].rolling(window=20).std()
        # Ensure bb_std is a Series, not DataFrame
        if hasattr(bb_std, 'iloc'):
            bb_std = bb_std.iloc[:, 0] if bb_std.shape[1] > 1 else bb_std.squeeze()
        df['BB_Upper'] = df['BB_Middle'] + (bb_std * 2)
        df['BB_Lower'] = df['BB_Middle'] - (bb_std * 2)
        df['BB_Width'] = df['BB_Upper'] - df['BB_Lower']
        
        # Handle division by zero for BB_Position
        bb_width_safe = df['BB_Width'].replace(0, np.nan)
        bb_position_calc = (df['Close'] - df['BB_Lower']) / bb_width_safe
        df['BB_Position'] = bb_position_calc.fillna(0.5)  # Default to middle position
        
        # Volume indicators
        df['Volume_SMA'] = df['Volume'].rolling(window=20).mean()
        volume_sma_safe = df['Volume_SMA'].replace(0, np.nan)
        volume_ratio_calc = df['Volume'] / volume_sma_safe
        df['Volume_Ratio'] = volume_ratio_calc.fillna(1.0)  # Default to average volume
        ...
        if self.symbol in ['NVDA', 'AMD', 'INTC']:  # Semiconductor stocks
            high_low_ratio_calc = df['High'] / df['Low'].replace(0, np.nan)
            df['High_Low_Ratio'] = high_low_ratio_calc.fillna(1.0)
        ...
        df = df.fillna(0)  # Final fallback for any remaining NaN
    ...
    def prepare_data(self, data):
        ...
        if np.any(np.isnan(features)) or np.any(np.isinf(features)):
            print("Warning: NaN or infinite values found in features, cleaning...")
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python fixed_trainer.py <SYMBOL>")
        sys.exit(1)
    ...
    if len(X) < 100:
        print(f"Not enough data for training {symbol} (need at least 100 samples, got {len(X)})")
        sys.exit(1)
